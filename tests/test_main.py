...
class TestSubtitleRecovery(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Set environment variables
        self.original_recycled_dir = os.environ.get("RECYCLED_MOVIES_DIRECTORY")
        self.original_recovered_dir = os.environ.get("RECOVERED_MOVIES_DIRECTORY")
    ...
    def tearDown(self):
        ...
        if self.original_recycled_dir is not None:
            os.environ["RECYCLED_MOVIES_DIRECTORY"] = self.original_recycled_dir
        ...
        if self.original_recovered_dir is not None:
            os.environ["RECOVERED_MOVIES_DIRECTORY"] = self.original_recovered_dir
    ...
    def test_recover_subtitle_folders_dry_run(self):
        ...
        self.assertIn("folders_moved", data)
        self.assertIn("subtitle_files_moved", data)
        self.assertIn("errors", data)
        self.assertIn("folders_with_subtitles", data)
        self.assertIn("moved_folders", data)
        self.assertIn("error_details", data)

        # Check expected results (dry run)
        self.assertTrue(data["dry_run"])
        self.assertEqual(data["folders_scanned"], 2)
        self.assertEqual(data["folders_with_subtitles_found"], 1)  # Only Movie1
        self.assertEqual(data["folders_moved"], 1)
        self.assertEqual(data["subtitle_files_moved"], 1)  # Only subtitle.srt
        self.assertEqual(data["errors"], 0)
    ...
    def test_recover_subtitle_folders_actual_move(self):
        """Test subtitle recovery endpoint with actual folder moving"""
        # Create folder with subtitle in root
        folder_with_subtitle = self.recycled_dir / "Movie1"
        folder_with_subtitle.mkdir()
        (folder_with_subtitle / "movie.mp4").touch()  # Media file - should not move
        (folder_with_subtitle / "subtitle.srt").touch()  # Subtitle file - should move
        (folder_with_subtitle / "poster.jpg").touch()  # Image file - should not move
        (folder_with_subtitle / "info.nfo").touch()  # Other file - should move

        # Create subdirectory with subtitle
        (folder_with_subtitle / "subs").mkdir()
        (folder_with_subtitle / "subs" / "subtitle2.srt").touch()  # Should move
        ...
        self.assertEqual(data["folders_moved"], 1)
        self.assertEqual(data["subtitle_files_moved"], 2)  # subtitle.srt and subtitle2.srt
        self.assertEqual(data["errors"], 0)

        # Verify folder was moved to recovered directory
        self.assertTrue((self.recovered_dir / "Movie1").exists())
        self.assertTrue((self.recovered_dir / "Movie1" / "subtitle.srt").exists())
        self.assertTrue((self.recovered_dir / "Movie1" / "subs" / "subtitle2.srt").exists())
        self.assertTrue((self.recovered_dir / "Movie1" / "info.nfo").exists())

        # Verify media files were NOT moved (should still be in recycled or removed)
        self.assertFalse((self.recovered_dir / "Movie1" / "movie.mp4").exists())
        self.assertFalse((self.recovered_dir / "Movie1" / "poster.jpg").exists())

        # Verify original folder structure is cleaned up (empty folders removed)
        # The folder should be removed if all files were moved
        self.assertFalse((self.recycled_dir / "Movie1").exists())
    ...
    def test_recover_subtitle_folders_multiple_subtitle_formats(self):
        ...
        self.assertTrue((self.recovered_dir / "Movie1" / "subtitle.srt").exists())
        self.assertTrue((self.recovered_dir / "Movie1" / "subtitle.ass").exists())
        self.assertTrue((self.recovered_dir / "Movie1" / "subtitle.vtt").exists())
        self.assertTrue((self.recovered_dir / "Movie1" / "subtitle.sub").exists())
    ...
    def test_recover_subtitle_folders_no_subtitles(self):
        ...
        self.assertEqual(data["folders_moved"], 0)
    ...
    def test_recover_subtitle_folders_metrics(self):
        ...
        self.assertIn("brronson_recovery_operation_duration_seconds", metrics_text)

        # Use the resolved path format
        recycled_path_resolved = normalize_path_for_metrics(self.recycled_dir)
        recovered_path_resolved = normalize_path_for_metrics(self.recovered_dir)
    ...
    def test_recover_subtitle_folders_preserves_structure(self):
        ...
        self.assertTrue((self.recovered_dir / "Movie1" / "subtitle.srt").exists())
        self.assertTrue(
            (self.recovered_dir / "Movie1" / "subs" / "en" / "subtitle.srt").exists()
        )
        self.assertTrue(
            (self.recovered_dir / "Movie1" / "subs" / "fr" / "subtitle.srt").exists()
        )

        self.assertEqual(data["subtitle_files_moved"], 3)

    def test_recover_subtitle_folders_skips_media_files(self):
        """Test that subtitle recovery skips media files and images"""
        folder = self.recycled_dir / "Movie1"
        folder.mkdir()
        ...
        # Only subtitle should be moved
        self.assertEqual(data["subtitle_files_moved"], 1)

        # Verify media files are NOT in recovered directory
        self.assertFalse((self.recovered_dir / "Movie1" / "movie.mp4").exists())
        self.assertFalse((self.recovered_dir / "Movie1" / "movie.avi").exists())
        self.assertFalse((self.recovered_dir / "Movie1" / "poster.jpg").exists())
        self.assertFalse((self.recovered_dir / "Movie1" / "poster.png").exists())
